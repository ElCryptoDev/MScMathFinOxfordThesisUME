...
# list = ['ETH', 'BTC',
#         'XMR', 'XRP', 'LTC', 'EOS', 'BCH', 'ETC', 'XLM', 'DOGE', 'DASH', 'ZEC', 'IOTA', 'NEO', 'BTG',
#         'TRX', 'XVG', 'VET', 'QTUM']
...
for coin in list:
    ...
    for sample in ['train']:
        ...
        #df_ts_1h.index = df_ts_1h.index.to_pydatetime()
        ...
        chunk_size = 10
        j = 1
        length = str(len(range(1, len(columns_to_plot), chunk_size)))
        for i in range(1, len(columns_to_plot), chunk_size):
            filename = config.storage_path + '\\Analysis\\CorrPlots\\10m' + '\\sample_' + sample + '_' + coin + '_10m_vars_' + str(
                i) + '_to_' + str(i + chunk_size) + '_corr.png'
            columns_to_plot_filter = [columns_to_plot[0]] + columns_to_plot[i:i + chunk_size-1]
            corrMatrix = df_ts_10m[columns_to_plot_filter].corr()
            name = 'Correlations ' + coin + ' ' + sample + ' 10m '+ ' (' + str(j) + '/' + length + ')'
            sn.heatmap(corrMatrix, annot=True).set(title=name)
            plt.tight_layout()
        ...
        j=1
        length = str(len(range(1, len(columns_to_plot), chunk_size)))
        for i in range(2, len(columns_to_plot), chunk_size):
            filename = config.storage_path + '\\Analysis\\CorrPlots\\1h' + '\\sample_' + sample + '_' + coin + '_1h_vars_' + str(
                i) + '_to_' + str(i + chunk_size) + '_corr.png'
            columns_to_plot_filter = [columns_to_plot[1]] + columns_to_plot[i:i + chunk_size - 1]
            corrMatrix = df_ts_1h[columns_to_plot_filter].corr()
            name = 'Correlations ' + coin + ' ' + sample + ' 1h '+ ' (' + str(j) + '/' + length + ')'
            sn.heatmap(corrMatrix, annot=True).set(title=name)
            plt.tight_layout()
            plt.savefig(filename, format='png')
            plt.close()
        ...
        j = 1
        length = str(len(range(1, len(columns_to_plot), chunk_size)))
        for i in range(3, len(columns_to_plot), chunk_size):
            filename = config.storage_path + '\\Analysis\\CorrPlots\\1d' + '\\sample_' + sample + '_' + coin + '_1d_vars_' + str(
                i) + '_to_' + str(i + chunk_size) + '_corr.png'
            columns_to_plot_filter = [columns_to_plot[2]] + columns_to_plot[i:i + chunk_size - 1]
            corrMatrix = df_ts_1d[columns_to_plot_filter].corr()
            name = 'Correlations ' + coin + ' ' + sample + ' 1d ' + ' (' + str(j) + '/' + length + ')'
            sn.heatmap(corrMatrix, annot=True).set(title=name)
            plt.tight_layout()
            plt.savefig(filename, format='png')
            plt.close()

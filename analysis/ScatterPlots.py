...
for col in columns_to_plot[3:]:
    ax = sn.scatterplot(x=col, y='RETURN_10M', data=df_ts_10m)
    plt.tight_layout()
    filename = config.storage_path + '\\Analysis\\ScatterPlots\\10m' + '\\sample_' + sample + '_10m' + '_' + coin + '_' + col + '_dist.png'
...
for col in columns_to_plot[3:]:
    ax = sn.scatterplot(x=col, y='RETURN_1H', data=df_ts_1h)
    plt.tight_layout()
    filename = config.storage_path + '\\Analysis\\ScatterPlots\\1h' + '\\sample_' + sample + '_1h'+ '_' + coin + '_' + col + '_dist.png'
    plt.savefig(filename, format='png')
    plt.close()
...
for col in columns_to_plot[3:]:
    ax = sn.scatterplot(x=col, y='RETURN_1D', data=df_ts_1d)
    plt.tight_layout()
    filename = config.storage_path + '\\Analysis\\ScatterPlots\\1d' + '\\sample_' + sample +'_1d'+ '_' + coin + '_' + col + '_dist.png'
    plt.savefig(filename, format='png')
    plt.close()
